...
**HARI KE-13** – Hubungan yang Kompleks
...
export const checkout = async (userId: number, items: { productId: number; quantity: number }[]) => {
  // 1. Hitung total harga (Idealnya fetch harga product dari DB dulu)
  // Logic disederhanakan: Anggap kita query harga dulu atau dikirim dari FE (Not secure, but for example)
  // Best practice: Fetch product prices inside transaction or before.
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const newTransaction = await tx.transaction.create({
      data: {
        userId,
        total: 100000, // Contoh hardcoded, harusnya hitungan real
      },
    });

    // 3. Buat Detail Item (Looping insert ke pivot table)
    for (const item of items) {
      await tx.transactionItem.create({
        data: {
          ...
          priceAtTime: 50000, // Contoh harga saat ini
        },
      });
    }
...
**Tips:** Pisahkan logic database (Prisma) dari logic HTTP (Req/Res) agar kodemu rapi dan mudah di-test. 💰
