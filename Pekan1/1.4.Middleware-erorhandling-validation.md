...
app.get('/', (req: Request, res: Response) => {
  ...
  successResponse(res, `Halo ${(req as any).namaSantri}! API E-Commerce – Hari 4`, { // Diubah dari Hari 3 ke Hari 4
...
app.listen(PORT, () => {
  console.log(`Server E-Commerce HARI 4 jalan di http://localhost:${PORT}`); // Diubah dari Hari 3 ke Hari 4
...
Kembangkan API E-Commerce kamu dari Hari 3 jadi seperti ini: // Diubah dari Hari 2 ke Hari 3
...
3.  Semua error (produk tidak ada, field kurang, route salah) harus:
...
5.  **Integrasikan Validasi Input dengan `express-validator`** untuk `POST /api/products` dan `GET /api/products/:id`.
...
*   [ ] Cari produk ID 999 → error 404 rapi
*   [ ] POST tanpa nama produk (atau dengan harga <= 0) → error 400 rapi dengan detail validasi
*   [ ] GET `/api/products/abc` → error 400 karena ID bukan angka
*   [ ] Ada waktu proses di setiap response (optional, bisa di `successResponse`)
*   [ ] Semua response pakai `successResponse` / `errorResponse`
*   [ ] Commit message: `"Hari 4: Middleware + Global Error Handler + Async Handler + Validasi (E-Commerce)"` // Diubah dari Hari 3 ke Hari 4

### Bonus (Kalau Cepat)

*   Buat validasi untuk `query` di `GET /api/products` (misal `page`, `limit`).
...
**“Membangun API E-Commerce yang kuat dengan Middleware, Error Handling, dan Validasi”** // Diubah dari kalimat sebelumnya
