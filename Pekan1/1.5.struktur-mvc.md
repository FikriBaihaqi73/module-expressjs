...
Hari ini kita **REFACTOR TOTAL** proyek E-Commerce dari Hari 3–4 menjadi struktur MVC + Service yang bersih dan scalable!
...
### TUGAS HARIAN HARI KE-5 (TANTANGAN MVC!)

Karena kode untuk **Products** sudah saya berikan lengkap di atas sebagai contoh, tugas kamu adalah **menduplikasi pola MVC tersebut untuk fitur baru**.

**1. Implementasi Ulang (Wajib)**
*   Salin dan jalankan kode materi di atas (Product MVC) di project kamu sampai benar-benar jalan.
*   Pahami alur datanya: Route -> Controller -> Service -> Model.

**2. Tantangan Utama: Buat Resource "Categories"**
Buatlah fitur manajemen Kategori Produk dengan struktur MVC yang sama persis!
*   **Model**: `src/models/category.model.ts` (Data: `id`, `nama`).
*   **Service**: `src/services/category.service.ts` (Logika: Get All, Create, Delete).
*   **Controller**: `src/controllers/category.controller.ts` (Pakai `successResponse` & `asyncHandler`).
*   **Route**: `src/routes/category.route.ts` (Endpoint: `GET /api/categories`, `POST /api/categories`, `DELETE /api/categories/:id`).
*   **App**: Daftarkan route baru di `app.ts` (`app.use('/api/categories', categoryRoutes)`).

**3. Tantangan Modifikasi: Tambah Stok Produk**
*   Update `Product` model: tambahkan field `stok` (number).
*   Update Validasi (`product.validation.ts`): `stok` wajib diisi dan harus angka positif.
*   Update Service & Controller: Pastikan `stok` bisa disimpan dan diupdate.
...
*   [ ] Struktur folder MVC rapi.
*   [ ] Endpoint Product berjalan normal (CRUD + Search).
*   [ ] **Endpoint Category berjalan normal** (Bisa tambah & lihat kategori).
*   [ ] **Ada validasi untuk Category** (nama kategori tidak boleh kosong).
*   [ ] **Produk punya field `stok`** dan tersimpan dengan benar.
*   [ ] Commit message: `"Hari 5: Implementasi MVC Product & Category"`