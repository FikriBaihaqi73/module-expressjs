...
export class BookService {
  ...
  static create(data: { judul: string; penulis: string; harga: number }) {
    if (!data.judul || !data.penulis || !data.harga) {
      throw new Error('Judul, penulis, dan harga wajib diisi');
    }
    const bukuBaru = {
      id: books.length + 1,
      ...data
...
2. Pindahkan semua kode dari `src/index.ts` lama ke file-file baru
3. Pastikan semua route masih berjalan sama seperti Hari 3
4. Tambahkan 1 route baru:
   - `GET /api/buku/search?judul=...` → cari berdasarkan judul (buat di Service!)
5. Tambahkan validasi di Service: harga harus > 0
