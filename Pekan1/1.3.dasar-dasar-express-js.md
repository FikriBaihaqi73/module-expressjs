...
// - Route Params (`/api/buku/:id`): Digunakan untuk mengidentifikasi sumber daya tertentu dalam koleksi. 
//   Contoh: ID buku, ID pengguna. Bagian dari URL path.
// - Query String (`/api/cari?penulis=...`): Digunakan untuk filter, sorting, atau pagination 
...
app.put('/api/buku/:id', (req: Request, res: Response) => {
  ...
  // `...books[index]` akan menyalin semua properti dari objek buku yang sudah ada.
  // `...req.body` akan menyalin semua properti dari data yang dikirimkan di body request.
  // Jika ada properti yang sama, properti dari `req.body` akan menimpa properti dari `books[index]`.
...
- [ ] Semua response format JSON yang sama:
  ...
  ```
- [ ] Commit message: `"Hari 3: Selesai semua routing CRUD + query + params"`
- [ ] Push ke GitHub
