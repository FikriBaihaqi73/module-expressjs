...
**HARI KE-11** – Aplikasi yang Tangguh (Robust)
...
Buat file `src/middlewares/error.handler.ts`.
...
Kemudian registrasikan di `src/app.ts` (paling bawah, setelah semua route):
...
```
Import di paling atas `src/index.ts` (atau `src/app.ts`):
```ts
...
Update file `src/middlewares/error.handler.ts`:
```ts
...
### Implementasi
Lakukan perubahan ini di Service, misal `src/services/product.service.ts`.
...
2.  **Global Error Handler:** Buat middleware error di `src/middlewares/error.handler.ts` dan panggil di `src/app.ts` di baris terakhir.
...
**Tips:** Error handling membedakan aplikasi amatir dan profesional. User tidak perlu tahu detail error teknis, tapi developer butuh log-nya. 🛡️
